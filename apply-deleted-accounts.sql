-- ===================================
-- تطبيق نظام الحسابات المحذوفة - نسخة سريعة
-- قم بتنفيذ هذا الملف في Supabase SQL Editor
-- ===================================

-- 1. إنشاء جدول الحسابات المحذوفة
...
-- 5. دالة حذف المستخدم مع الأرشفة
...
BEGIN
    ...
    -- حساب عدد الدعوات المرتبطة
    SELECT COUNT(*) INTO deleted_inv_count
    FROM invitations
    WHERE email = user_record.email;
    ...
    RETURN jsonb_build_object(
        ...
        'deleted_invitations_count', deleted_inv_count
    );
...
-- ===================================
-- يمكنك الآن:
-- 1. حذف أعضاء الفريق من لوحة التحكم
-- 2. عرض الحسابات المحذوفة
-- 3. حذف الحسابات نهائياً
-- ===================================
